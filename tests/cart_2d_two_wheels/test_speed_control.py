...
class Cart2DRobot(RoboticSystem):
    ...
    def run(self):
        ...
        # same vref
        Tleft = self.left_controller.evaluate(self.delta_t, self.vref, vl)
        Tright = self.right_controller.evaluate(self.delta_t, self.vref, vr)
        ...
        self.plotter.add('vref', self.vref)
        if self.t > 2:
